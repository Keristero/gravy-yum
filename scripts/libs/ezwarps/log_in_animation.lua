...
function create_jack_in_out_animation(is_arriving)
    local log_in_animation = {
        ...
        animate=function(player_id)
            ...
            Net.provide_asset(area_id, "/server/assets/landings/logout.png")
            Net.provide_asset(area_id, "/server/assets/landings/logout.animation")
            local warp_in_effect_id = "warp_in_effect_"..player_id
            print('spawning log in effect bot')
            Net.create_bot(warp_in_effect_id, {
                warp_in = false,
                texture_path = "/server/assets/landings/logout.png",
                animation_path = "/server/assets/landings/logout.animation",
                area_id = area_id,
                x = player_pos.x+0.2,
                y = player_pos.y+0.2,
                z = player_pos.z
            ...
            if is_arriving then
                ...
                Net.play_sound(area_id, '/server/assets/landings/log_in.ogg')
            else
                Net.animate_bot(warp_in_effect_id, "JACK_OUT", false)
                Net.play_sound(area_id, '/server/assets/landings/log_out.ogg')
            end
            local duration = 1.0
